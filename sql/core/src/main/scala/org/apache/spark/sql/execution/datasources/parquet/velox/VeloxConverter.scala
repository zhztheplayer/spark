...
class VeloxConverter private (
  ...
  def filterToVeloxExpr(scanRowType: RowType, filter: Filter): TypedExpr = {
    ...
    def fieldAccess(attribute: String): TypedExpr = {
      attributeToExprLookUp(attribute)
    }

    def fieldType(attribute: String): Type = {
      attributeToExprLookUp(attribute).getReturnType
    }
...
object VeloxConverter {
  def of(session: Session, bufferAllocator: BufferAllocator, timeZoneId: String): VeloxConverter = {
    new VeloxConverter(session, bufferAllocator, timeZoneId)
  }
}
