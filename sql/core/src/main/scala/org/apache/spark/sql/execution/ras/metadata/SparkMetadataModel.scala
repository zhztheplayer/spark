 ...
 */
package org.apache.spark.sql.execution.ras.metadata
...
object SparkMetadataModel extends MetadataModel[SparkPlan] {
  ...
  override def dummy(): Metadata = SparkMetadata(Schema(List()), LogicalLink.notFound)
  override def verify(one: Metadata, other: Metadata): Unit = (one, other) match {
    case (left: SparkMetadata, right: SparkMetadata) =>
  ...
  override def assignToGroup(group: GroupLeafBuilder[SparkPlan], meta: Metadata): Unit =
    (group, meta) match {
      case (builder: GroupLeafExec.Builder, metadata: SparkMetadata) =>
